...
namespace DAO_QuanLy
{
    public class PhieuSuaChuaDAO
    {
        ...
        {
            string query = "Select * from PHUTUNG where MaPhuTung = " + maVTPT;
        ...
        public void LuuPhieuSuaChua(string BienSo, int TienCong, int TienPhuTung, int TongTien, DataTable TC, DataTable VTPT)//Lưu dữ liệu được nhập vào 2 bảng PHIEUSUACHUA và CHITIETPHIEUSUACHUA
        {
            ...
            if (slvtpt >= sltc)
            {
                ...
                {
                    query = "insert into CTPHIEUSUACHUA values (" + ma + " , " + TC.Rows[i][3].ToString() + " , " + VTPT.Rows[i][5].ToString() + " , " + VTPT.Rows[i][2] + ")";
                    re = DataProvider.Instance.ExecuteNonQuery(query);
                }
                for (int i = sltc; i < slvtpt; i++)
                {
                    query = "insert into CTPHIEUSUACHUA ( MaPhieuSuaChua, MaPhuTung, SoLuongPhuTung) values (" + ma + " , " + VTPT.Rows[i][5].ToString() + " , " + VTPT.Rows[i][2] + ")";
                    re = DataProvider.Instance.ExecuteNonQuery(query);
                }
            }
            else
            {
                ...
                for (int i = slvtpt; i < sltc; i++)
                {
                    ...
                    query = "insert into CTPHIEUSUACHUA (MaPhieuSuaChua, MaTC) values (" + ma + " , " + TC.Rows[i][3].ToString() + ")";
                    re = DataProvider.Instance.ExecuteNonQuery(query);
                }
            }
        ...
        public void NhapVTPT(DataTable VTPT)//Thực hiện cập nhật lại số lượng vtpt còn lại trong kho.
        {
            ...
            {
                query = "Update PHUTUNG set SoLuong = SoLuong - " + VTPT.Rows[i][2] + "where MaPhuTung = " + VTPT.Rows[i][5];
                int re = DataProvider.Instance.ExecuteNonQuery(query);
                i = i + 1;
            }
        }
