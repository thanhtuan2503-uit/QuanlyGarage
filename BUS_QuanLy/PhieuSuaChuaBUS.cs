...
namespace BUS_QuanLy
{
    ...
    public class PhieuSuaChuaBUS
    {
        ...
        {
            return DAO_QuanLy.PhieuSuaChuaDAO.Instance.TaoDataTable(a, name);
        }
        ...
        public DataTable ThemHangVTPT(DataTable dt, string VTPT, string Soluong, int DonGia, string MaPhuTung)//Thêm 1 hàng cho dt VTPT với các nội dung nhập vào
        {
            ...
            return DAO_QuanLy.PhieuSuaChuaDAO.Instance.ThemHangVTPT(dt, a, VTPT, sl, DonGia, MaPhuTung);
        }
        ...
        {
            int a = dt.Rows.Count + 1;
            return DAO_QuanLy.PhieuSuaChuaDAO.Instance.ThemHangTienCong(dt, a, NoiDung, ChiPhi, MaTC);
        }

        public int LayDonGiaVTPT(string maVTPT)//Lấy đơn giá của VTPT theo mã
        {
            return DAO_QuanLy.PhieuSuaChuaDAO.Instance.LayDonGiaVTPT(maVTPT);
        }

        public int LayChiPhiTienCong(string maTienCong)//Lấy chi phí của tiền công theo mã
        {
            return DAO_QuanLy.PhieuSuaChuaDAO.Instance.LayChiPhiTienCong(maTienCong);
        }

        public string LayNoiDungTienCong(string maTienCong)//Lấy chi phí của tiền công theo mã
        {
            return DAO_QuanLy.PhieuSuaChuaDAO.Instance.LayNoiDungTienCong(maTienCong);
        }
        ...
        {
            DAO_QuanLy.PhieuSuaChuaDAO.Instance.LuuPhieuSuaChua(BienSo, TienCong, TienCong, TongTien, TC, VTPT);

        }

        public void NhapVTPT(DataTable VTPT)//Thực hiện cập nhật lại số lượng vtpt còn lại trong kho.
        {
            DAO_QuanLy.PhieuSuaChuaDAO.Instance.NhapVTPT(VTPT);
        }
        ...
        public bool KiemTraSoLuong(string MaVTPT, int SoLuong)//Kiểm tra số lượng nhập vào có nằm trong khả năng đáp ứng của kho.
        {
            ...
            if (!TonTai)
            {
                return DAO_QuanLy.PhieuSuaChuaDAO.Instance.KiemTraSoLuong(VTPTDangNhap, MaVTPT, SoLuong);
            }
            return true;
        }
        ...
        {
            DAO_QuanLy.PhieuSuaChuaDAO.Instance.CapNhatTienNo(BienSo, TienSuaChua);
        }
    }
}
